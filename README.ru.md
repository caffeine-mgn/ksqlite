...
Публикуемая версия берётся из переменной окружения `GITHUB_REF_NAME` в CI
(значение тега, например `0.1.3`), а локально откатывается на `0.1.3`.

## Установка

Добавьте зависимость в ваш KMP-модуль (Gradle Kotlin DSL, 0.1.3):
